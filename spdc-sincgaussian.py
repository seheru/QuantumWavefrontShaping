...
# Plot 1: Joint Momentum Distribution (JMD)
im0 = axs[0, 0].imshow(JMD, origin='lower', cmap='inferno',
                       extent=[-q_max*q_scale, q_max*q_scale, -q_max*q_scale, q_max*q_scale])
axs[0, 0].set_title(r"$\mathbf{(a)\ Momentum\ Space\ |\Psi(q_s, q_i)|^2\ (Sinc\text{-}Gauss)}$", fontsize=11)
...
axs[0, 0].axline((0, 0), slope=-1, color='cyan', linestyle='--', alpha=0.6, label=r"$q_s = -q_i$")
axs[0, 0].legend(loc='upper right', fontsize=9)
fig.colorbar(im0, ax=axs[0, 0], fraction=0.046, pad=0.04, label="Normalized Intensity")

# Plot 2: Joint Position Distribution (JPD via 2D-FFT)
...
im1 = axs[0, 1].imshow(JPD_sub, origin='lower', cmap='viridis',
                       extent=[-x_limit, x_limit, -x_limit, x_limit])
axs[0, 1].set_title(r"$\mathbf{(b)\ Position\ Space\ |\Psi(x_s, x_i)|^2\ (2D\text{-}FFT)}$", fontsize=11)
...
axs[0, 1].axline((0, 0), slope=1, color='yellow', linestyle='--', alpha=0.6, label=r"$x_s = x_i$")
axs[0, 1].legend(loc='upper left', fontsize=9)
fig.colorbar(im1, ax=axs[0, 1], fraction=0.046, pad=0.04, label="Normalized Intensity")

# Plot 3: Momentum Cross-Section along anti-diagonal qs = -qi (Shows Sinc Rings)
diag_anti_q = np.diagonal(np.fliplr(JMD))
q_axis_anti = np.linspace(-q_max, q_max, N_pts) * q_scale

axs[1, 0].plot(q_axis_anti, diag_anti_q, 'r-', lw=2, label="Anti-diagonal ($q_s = -q_i$)")
axs[1, 0].set_title(r"$\mathbf{(c)\ Momentum\ Profile\ along\ } q_s = -q_i$", fontsize=11)
axs[1, 0].set_xlabel(r"Transverse Momentum $q\ [\mathrm{rad/\mu m}]$", fontsize=11)
axs[1, 0].set_ylabel("Intensity [Linear]", fontsize=11)
axs[1, 0].grid(True, alpha=0.3)
axs[1, 0].legend(fontsize=9)

# Inset / Log-Scale plot to highlight Sinc Sidelobes
ax_inset = axs[1, 0].inset_axes([0.60, 0.42, 0.36, 0.5])
...
# Plot 4: Position Cross-Sections (Conditional vs Diagonal)
mid_idx = N_pts // 2
diag_coinc_x = np.diagonal(JPD)

axs[1, 1].plot(x_vec * x_scale, JPD[mid_idx, :], 'g-', lw=2, label=r"Conditional $P(x_s | x_i=0)$ (Narrow)")
axs[1, 1].plot(x_vec * x_scale, diag_coinc_x, 'm--', lw=2, label=r"Coincidence $x_s = x_i$ (Pump Envelope)")
axs[1, 1].set_title(r"$\mathbf{(d)\ Position\ Profiles\ (Spatial\ Coincidence)}$", fontsize=11)
axs[1, 1].set_xlim([-x_limit, x_limit])
axs[1, 1].set_xlabel(r"Transverse Position $x\ [\mathrm{\mu m}]$", fontsize=11)
